...
void ConnectToDatabase() {
    ...
    // Allocate a context
    g_retcode = /* CONVERTED FROM SYBASE: cs_ctx_alloc(CS_VERSION_125, &g_context) */
g_context = Environment::createEnvironment()
/* END CONVERSION */;
    if (g_retcode != SUCCESS) {
    ...
    g_retcode = /* CONVERTED FROM SYBASE: ct_init(g_context, CS_VERSION_125) */
...
/* END CONVERSION */;
    ...
    g_retcode = /* CONVERTED FROM SYBASE: ct_con_alloc(g_context, &g_connection) */
g_connection = g_context->createConnection(username, password, connectionString)
/* END CONVERSION */;
    if (g_retcode != SUCCESS) {
    ...
    g_retcode = ct_connect(g_connection, server, -1);
    if (g_retcode != SUCCESS) {
    ...
    g_retcode = /* CONVERTED FROM SYBASE: ct_cmd_alloc(g_connection, &g_command) */
g_command = g_connection->createStatement()
/* END CONVERSION */;
    if (g_retcode != SUCCESS) {
    ...
    g_retcode = /* CONVERTED FROM SYBASE: ct_send(g_command) */
g_command->execute()
/* END CONVERSION */;
    if (g_retcode != SUCCESS) {
    ...
    // Process all results
    int result_type;
    while ((g_retcode = /* CONVERTED FROM SYBASE: ct_results(g_command, &result_type) */
result_type = (g_command->getResultSet() != NULL)
...
/* END CONVERSION */;
    ...
    int id_data;
    char name_data[100];
    char address_data[200];
    float balance_data;
    Type id_fmt, name_fmt, address_fmt, balance_fmt;
...
/* END CONVERSION */) == SUCCESS) {
        if (result_type == CS_ROW_RESULT) {
            ...
            balance_fmt.maxlength = sizeof(float);
            ...
            // Fetch the row
            if (/* CONVERTED FROM SYBASE: ct_fetch(g_command, 0, 0, 0) */
g_command->getResultSet()->next()
/* END CONVERSION */ == SUCCESS) {
            ...
            while (/* CONVERTED FROM SYBASE: ct_fetch(g_command, 0, 0, 0) */
g_command->getResultSet()->next()
/* END CONVERSION */ == SUCCESS) {
                // Do nothing
            }
...
void DisconnectFromDatabase() {
    ...
    g_retcode = ct_close(g_connection, 0);
    if (g_retcode != SUCCESS) {
    ...
    // Drop the connection
    g_retcode = /* CONVERTED FROM SYBASE: ct_con_drop(g_connection) */
Environment::terminateConnection(g_connection)
/* END CONVERSION */;
    if (g_retcode != SUCCESS) {
    ...
    // Drop the context
    g_retcode = /* CONVERTED FROM SYBASE: cs_ctx_drop(g_context) */
