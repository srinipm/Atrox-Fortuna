...
public class GlobalStockLendingSystem implements AutoCloseable {
    ...
    private void initializeConnectionPool() throws SQLException {
        try {
            ...
            Class.forName("com.sybase.jdbc4.jdbc.SQLServerDriver");
            ...
            // MSSQL doesn\'t use LANGUAGE property;
    ...
    public int runEndOfDayProcessing(LocalDate processingDate) throws SQLException {
        ...
        try {
            ...
            try (/* CONVERTED FROM SYBASE: /* CONVERTED FROM SYBASE: CallableStatement cstmt = conn.prepareCall("{call update_market_values(?)}") */
CallableStatement cstmt = conn.prepareCall("{call update_market_values(?)}")  // MSSQL uses same syntax
/* END CONVERSION */ */
CallableStatement cstmt = conn.prepareCall("{{call update_market_values(?)}}")  // MSSQL uses same syntax
/* END CONVERSION */) {
            ...
            try (/* CONVERTED FROM SYBASE: /* CONVERTED FROM SYBASE: CallableStatement cstmt = conn.prepareCall("{call calculate_daily_fees(?)}") */
CallableStatement cstmt = conn.prepareCall("{call calculate_daily_fees(?)}")  // MSSQL uses same syntax
/* END CONVERSION */ */
CallableStatement cstmt = conn.prepareCall("{{call calculate_daily_fees(?)}}")  // MSSQL uses same syntax
/* END CONVERSION */) {
                cstmt.setDate(1, java.sql.Date.valueOf(processingDate));
            ...
            try (/* CONVERTED FROM SYBASE: /* CONVERTED FROM SYBASE: CallableStatement cstmt = conn.prepareCall("{call update_counterparty_exposure(?)}") */
CallableStatement cstmt = conn.prepareCall("{call update_counterparty_exposure(?)}")  // MSSQL uses same syntax
/* END CONVERSION */ */
CallableStatement cstmt = conn.prepareCall("{{call update_counterparty_exposure(?)}}")  // MSSQL uses same syntax
/* END CONVERSION */) {
                cstmt.setDate(1, java.sql.Date.valueOf(processingDate));
            ...
            try (/* CONVERTED FROM SYBASE: /* CONVERTED FROM SYBASE: CallableStatement cstmt = conn.prepareCall("{call generate_daily_reports(?)}") */
CallableStatement cstmt = conn.prepareCall("{call generate_daily_reports(?)}")  // MSSQL uses same syntax
/* END CONVERSION */ */
CallableStatement cstmt = conn.prepareCall("{{call generate_daily_reports(?)}}")  // MSSQL uses same syntax
/* END CONVERSION */) {
                cstmt.setDate(1, java.sql.Date.valueOf(processingDate));
